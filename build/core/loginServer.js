...
async function handle(req, res) {
    ...
    if (req.method === "GET" && url.pathname === "/login") {
        ...
        if (!item)
            return send(404, page("<h1>Ссылка устарела</h1><p class=\"sub\">Вызови login ещё раз</p>"));
    ...
    if (req.method === "POST" && url.pathname === "/login") {
        ...
        if (!item)
            return send(400, page("<h1>Форма устарела</h1><p class=\"sub\">Вызови login ещё раз</p>"));
        try {
            ...
            return send(400, formHtml(item, error.message));
        }
    }
    ...
    return new Promise((resolve, reject) => {
        ...
        srv.once("error", (error) => {
            ...
            if (error.code === "EADDRINUSE" && attempt < 20)
                return resolve(listen(port + 1, attempt + 1));
            reject(error);
        });
...
export async function startLogin(domain, baseUrlOverride) {
    ...
    if (project)
        return { status: "authorized", project };
