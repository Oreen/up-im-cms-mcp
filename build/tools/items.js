...
async function fetchItem(domain, node, id) {
    return api(domain, { path: `/admin/node/${node}/content/${id}` });
}
//read-merge-write: база из текущего состояния, поверх — только изменённые поля
export function mergeWire(base, patch, patchedFields) {
    const merged = new Map(base);
    for (const field of patchedFields)
        for (const key of wireKeysFor(field))
            merged.delete(key);
    for (const [key, parts] of patch)
        merged.set(key, parts);
    return merged;
}
...
export const itemSetFieldTool = defineTool({
    ...
    handler: async ({ domain, node, id, field, value }) => {
        ...
        if (!f)
            throw new ValidationError([`поле "${field}" не найдено; доступны: ${ctx.fields.map(x => x.code).join(", ")}`]);
...
export const itemDeleteTool = defineTool({
    ...
    handler: async ({ domain, node, ids }) => {
        const wire = new Map([["ids[]", ids.map(String)]]);
...
export const itemMoveTool = defineTool({
    ...
    handler: async ({ domain, node, ids, toNode }) => {
        const wire = new Map([["ids[]", ids.map(String)], ["toNode", [String(toNode)]]]);
...
export const itemCopyTool = defineTool({
    ...
    handler: async ({ domain, node, id, toNode }) => {
        const wire = new Map([["id", [String(id)]], ["toNode", [String(toNode)]]]);
        const saved = await api(domain, { method: "POST", path: `/admin/node/${node}/content/copy`, body: wire });
...
export const itemsExportCsvTool = defineTool({
    ...
    handler: async ({ domain, node, path, filters, sorter }) => {
        ...
        for (let page = 1;; page++) {
            ...
            const data = await api(domain, { path: `/admin/node/${node}/content`, query });
            all.push(...data.items);
            if (all.length >= data.totalItems || !data.items.length)
                break;
        }
...
export const itemsImportCsvTool = defineTool({
    ...
    handler: async ({ domain, node, path }) => {
        ...
        if (unknown.length)
            throw new ValidationError([`столбцы не соответствуют полям раздела: ${unknown.join(", ")}`, `допустимые заголовки: ${titles.join(", ")}`]);
        const wire = new Map([["file", [{ blob: new Blob([text], { type: "text/csv" }), name: "import.csv" }]]]);
