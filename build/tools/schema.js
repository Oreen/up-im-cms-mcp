...
import { describeField, getFields, getNode, getNodeParams, getNodeTypes, getOptions, isSelectTypeField } from "../core/schema.js";
...
export const schemaTool = defineTool({
    ...
    handler: async ({ domain, node, type }) => {
        if (!node && !type)
            throw new ValidationError(["укажи node или type"]);
        const nodeType = type ?? (await getNode(domain, node)).type;
        const fields = await getFields(domain, nodeType);
        const out = { type: nodeType, item_fields: fields.map(describeField) };
        if (node) {
...
async function resolveField(domain, node, code, kind) {
    ...
    if (!field)
        throw new ValidationError([`поле "${code}" не найдено; доступны: ${fields.map(f => f.code).join(", ")}`]);
    if (!isSelectTypeField(field))
        throw new ValidationError([`поле "${code}" типа ${field.field_type} не имеет справочника`]);
    return field;
}
