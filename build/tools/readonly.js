...
export const historySearchTool = defineTool({
    ...
    handler: async ({ domain, table, item_id, item_title, field, user, value, date_from, date_to, page, perPage }) => {
        const filters = {};
        if (table)
            filters.item_table = table;
        if (item_id !== undefined)
            filters.item_id = item_id;
        if (item_title)
            filters.item_title = item_title;
        if (field)
            filters.field = field;
        if (typeof user === "number")
            filters.user = user;
        else if (user)
            filters.user_title = user;
        if (value)
            filters.new_value = value;
        if (date_from)
            filters[">pub_date"] = date_from;
        if (date_to) {
        ...
        const data = await api(domain, { path: "/admin/history", query });
...
export const usersTool = defineTool({
    ...
    handler: async ({ domain, page, perPage, filters, sorter }) => {
        ...
        const data = await api(domain, { path: "/admin/user", query });
...
export const feedbackTool = defineTool({
    ...
    handler: async ({ domain, page, perPage, filters, sorter }) => {
        ...
        const data = await api(domain, { path: "/admin/feedback", query });
...
export const ordersTool = defineTool({
    ...
    handler: async ({ domain, page, perPage, filters, sorter }) => {
        ...
        const data = await api(domain, { path: "/admin/order", query });
...
export const orderProductsTool = defineTool({
    ...
    handler: async ({ domain, order }) => {
        const data = await api(domain, { path: `/admin/order/${order}/products` });
        return data.items;
    },
});
