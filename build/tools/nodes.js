...
import { agentToWire, displayToAgent, displayToWire, hasInlineMedia, wireKeysFor } from "../core/convert.js";
import { ValidationError } from "../core/errors.js";
import { writeBinaryFile } from "../core/files.js";
import { api, apiRaw, readImportSse, wireSet } from "../core/http.js";
import { getNode, getNodeParams } from "../core/schema.js";
...
function nodeField(code, title, field_type, required = false) {
...
function compactTree(nodes, baseUrl, depth) {
    return nodes.map(n => ({
        id: n.id,
        code: n.code,
        type: n.type,
...
export const nodeTreeTool = defineTool({
    ...
    handler: async ({ domain, parent, depth, type }) => {
        ...
        const tree = await api(domain, { path: type ? `/admin/node/type/${encodeURIComponent(type)}` : "/admin/node" });
        let roots = tree;
        if (parent) {
            const hit = findInTree(tree, parent);
            if (!hit)
                throw new ValidationError([`раздел ${parent} не найден`]);
            roots = hit.children ?? [];
        }
        return compactTree(roots, baseUrl, depth ?? 2);
    },
});
async function nodeView(domain, id) {
    const baseUrl = getProject(domain).baseUrl;
    const node = await getNode(domain, id);
    const params = await getNodeParams(domain, id);
    const view = displayToAgent(node, NODE_FIELDS, baseUrl);
    ...
    delete view.params.id;
    if (!node.public)
        view.note = "Раздел не опубликован — страница на сайте отдаст 404";
    return view;
}
...
export const nodeSaveTool = defineTool({
    ...
    handler: async ({ domain, id, fields, params, parent }) => {
        if (!id && !fields?.type)
            throw new ValidationError(["при создании раздела обязателен fields.type"]);
        if (!id && !fields?.title)
            throw new ValidationError(["при создании раздела обязателен fields.title"]);
        ...
        if (fields && Object.keys(fields).length) {
            const current = id ? (await getNode(domain, id)) : undefined;
            const patch = await agentToWire(fields, NODE_FIELDS, { domain, optionsKind: "fields", current });
            ...
            const saved = await api(domain, { method: "POST", path: "/admin/node/edit", body: wire });
            nodeId = saved.id;
        }
        if (!nodeId)
            throw new ValidationError(["нечего сохранять: укажи fields и/или params"]);
        if (parent) {
            const currentParent = id ? (await getNode(domain, nodeId)).parent : 0;
            if (currentParent !== parent) {
                const wire = new Map([["nodeId", [String(nodeId)]], ["upNodeId", [String(parent)]], ["isParent", ["true"]]]);
        ...
        if (params && Object.keys(params).length) {
            ...
            if (!paramFields.length)
                throw new ValidationError(["у этого типа раздела нет параметров"]);
...
export const nodesImportCsvTool = defineTool({
    ...
    handler: async ({ domain, path }) => {
        ...
        const wire = new Map([["file", [{ blob: new Blob([buffer], { type: "text/csv" }), name: "nodes.csv" }]]]);
